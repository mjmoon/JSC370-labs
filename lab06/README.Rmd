...
# Extracting the data using regex (both work)
...
_There are `r format(counts, big.digit = ",")` sars-cov-2 paperson PubMed (as of Feb 17, 2023)._
...
publications <- GET(
  ...
  query = list(
    ...
    retmax = 1000,
    rettype = "abstract",
    id = paste(ids, collapse = ",") # collapse elements in a vector with "," between them
    )
)
...
institution <- str_extract_all(
  publications_txt,
  # (\\w\\s?) captures "words" that either end with or without a space
  "University\\s+of\\s+(\\w\\s?\\-?)+|(\\w\\s?\\-?)+Institute\\s+of\\s+(\\w\\s?\\-?)+"
  )
...
schools_and_deps <- str_extract_all(
  publications_txt,
  "School\\s+of\\s+(\\w\\s?)+|Department\\s+of\\s+(\\w\\s?)+"
  )
schools_and_deps <- unlist(schools_and_deps)
schools_and_deps <- str_replace_all(schools_and_deps, "\\s+", " ")
table(schools_and_deps)
```


> Note: Capturing multiple words allowed long sentences and other unwanted longer terms. e.g., We want to capture "School of Veterinary Medicine" only instead of "School fo Veterinary Medicine at Tufts University". Implementing a finer regex or post-search filters/rules would improve the results.
...
# capture all HTML tags with or without arguments
abstracts <- str_remove_all(abstracts, '<(/)?[A-Za-z]+\\s?[A-Za-z0-9=\\"\\s]+>')
...
_`r sum(is.na(abstracts))` don't have an abstract._
...
# rmarkdown::paged_table(database) # doesn't show on github
```
...
```md
View Week 6 Lecture [here](https://cdn.jsdelivr.net/gh/JSC370/jsc370-2023@main/slides/JSC370-slides-06.pdf)
```
